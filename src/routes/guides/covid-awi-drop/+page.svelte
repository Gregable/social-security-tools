...
<main>
  <div>
    ...
    />
    <p>
      From this table, Social Security will calculate your AIME or Average
      Indexed Monthly Earnings. If you have 35 vaules, it's really just the
      average of all of the values in the "Indexed Earnings" column divided by
      12 to get from annual to monthly. The calculation is a little further down
      the page:
    </p>
    <img
      src="/covid-aime.jpg"
    ...
    <p>
      ...
      the years up to and including age 60, the multipler for a given year will
      be the Average Wage Index (AWI) in the year you turned 60 divided by the
      AWI of the multiplier year. So, for someone who turns 60 in year 2020:
    </p>
    ...
    <p>
      ...
      given year is not known until taxes are filed the following year. So the
      AWI for 2020 won't be known until late in 2021. By the start of 2022, we
      will know the AWI correctly.
    </p>
    <p>
    ...
    <p>
      ...
      (or 5.9% less than 2019 depending on what you are comparing it to). This
      is based on the Social Security Trustees report, so it is probably a good
      estimate.
    </p>
    <p>
      The number one really cares about is the Primary Insurance Amount (PIA),
      not AIME, since PIA is the number that benefits are based on. However,
      since the bend points in the PIA formula are also adjusted by AWI, the
      effect on PIA is also very close to the same as on AIME. The calculator
      provides no easy means for manually modifying bend points to demonstrate
      easily, but we can do so manually for this example.
    </p>
    <p>
      First, let's see what the calculator predicts as the PIA if there was not
      a covid related drop:
    </p>
    <img
      src="/covid-pia.jpg"
    ...
    />
    <p>
      Going from $1,977.60 to $1,798.40 is a 9.06% drop, very similar to the
      9.1% number we started with. In conclusion, a significant drop in AWI in a
      single year can have an similarly outsized effect on the Social Security
      benefits for those who turn 60 in that year. In the history of the Social
      Security program, there has not been such a dramatic drop in AWI from one
      year to the next like Covid appears to have produced, so this has never
      been much of an issue until now.
    </p>
  </div>
